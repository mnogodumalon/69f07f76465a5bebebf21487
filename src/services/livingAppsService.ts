...
import type { Stammdaten, Angebotsanfrage, CreateStammdaten, CreateAngebotsanfrage } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- ANGEBOTSANFRAGE ---
  static async getAngebotsanfrage(): Promise<Angebotsanfrage[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.ANGEBOTSANFRAGE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Angebotsanfrage[];
    return enrichLookupFields(records, 'angebotsanfrage');
  }
  static async getAngebotsanfrageEntry(id: string): Promise<Angebotsanfrage | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.ANGEBOTSANFRAGE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Angebotsanfrage;
    return enrichLookupFields([record], 'angebotsanfrage')[0];
  }
  static async createAngebotsanfrageEntry(fields: CreateAngebotsanfrage) {
    return callApi('POST', `/apps/${APP_IDS.ANGEBOTSANFRAGE}/records`, { fields: cleanFieldsForApi(fields as any, 'angebotsanfrage') });
  }
  static async updateAngebotsanfrageEntry(id: string, fields: Partial<CreateAngebotsanfrage>) {
    return callApi('PATCH', `/apps/${APP_IDS.ANGEBOTSANFRAGE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'angebotsanfrage') });
  }
  static async deleteAngebotsanfrageEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.ANGEBOTSANFRAGE}/records/${id}`);
  }
