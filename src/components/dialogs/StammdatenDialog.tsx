...
export function StammdatenDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: StammdatenDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "kunden_csv": string | null, // Kunden CSV\n  "ansprechpartner_csv": string | null, // Ansprechpartner CSV\n  "artikel_csv": string | null, // Artikel CSV\n  "bezeichnung": string | null, // Bezeichnung / Version\n  "stand_datum": string | null, // YYYY-MM-DD\n}`;
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="kunden_csv">Kunden CSV</Label>
            <Textarea
              id="kunden_csv"
              value={fields.kunden_csv ?? ''}
              onChange={e => setFields(f => ({ ...f, kunden_csv: e.target.value }))}
              rows={3}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="ansprechpartner_csv">Ansprechpartner CSV</Label>
            <Textarea
              id="ansprechpartner_csv"
              value={fields.ansprechpartner_csv ?? ''}
              onChange={e => setFields(f => ({ ...f, ansprechpartner_csv: e.target.value }))}
              rows={3}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="artikel_csv">Artikel CSV</Label>
            <Textarea
              id="artikel_csv"
              value={fields.artikel_csv ?? ''}
              onChange={e => setFields(f => ({ ...f, artikel_csv: e.target.value }))}
              rows={3}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="bezeichnung">Bezeichnung / Version</Label>
            <Input
              id="bezeichnung"
          ...
          <div className="space-y-2">
            <Label htmlFor="stand_datum">Stand-Datum</Label>
            <Input
              id="stand_datum"
              type="date"
              value={fields.stand_datum ?? ''}
              onChange={e => setFields(f => ({ ...f, stand_datum: e.target.value }))}
            />
          </div>
          <DialogFooter>
