...
const STAMMDATEN_FIELDS = [
  { key: 'kunden_csv', label: 'Kunden CSV', type: 'string/textarea' },
  { key: 'ansprechpartner_csv', label: 'Ansprechpartner CSV', type: 'string/textarea' },
  { key: 'artikel_csv', label: 'Artikel CSV', type: 'string/textarea' },
  { key: 'bezeichnung', label: 'Bezeichnung / Version', type: 'string/text' },
  { key: 'stand_datum', label: 'Stand-Datum', type: 'date/date' },
];
const ANGEBOTSANFRAGE_FIELDS = [
  { key: 'stammdaten_ref', label: 'Stammdaten (Referenz)', type: 'applookup/select', targetEntity: 'stammdaten', targetAppId: 'STAMMDATEN', displayField: 'bezeichnung' },
  { key: 'anfrage_pdf', label: 'Anfrage (PDF)', type: 'file' },
  { key: 'ergebnis_json', label: 'Ergebnis-JSON', type: 'string/textarea' },
  { key: 'notizen', label: 'Interne Notizen', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'stammdaten', label: 'Stammdaten', pascal: 'Stammdaten' },
  { key: 'angebotsanfrage', label: 'Angebotsanfrage', pascal: 'Angebotsanfrage' },
] as const;
...
export default function AdminPage() {
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'angebotsanfrage': return (data as any).angebotsanfrage as Angebotsanfrage[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'angebotsanfrage' && fieldKey === 'stammdaten_ref') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'angebotsanfrage': return ANGEBOTSANFRAGE_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'angebotsanfrage': return {
        create: (fields: any) => LivingAppsService.createAngebotsanfrageEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateAngebotsanfrageEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteAngebotsanfrageEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'angebotsanfrage' || dialogState?.entity === 'angebotsanfrage') && (
        <AngebotsanfrageDialog
          open={createEntity === 'angebotsanfrage' || dialogState?.entity === 'angebotsanfrage'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'angebotsanfrage' ? handleUpdate : (fields: any) => handleCreate('angebotsanfrage', fields)}
          defaultValues={dialogState?.entity === 'angebotsanfrage' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'angebotsanfrage' && (
        <AngebotsanfrageViewDialog
          open={viewState?.entity === 'angebotsanfrage'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'angebotsanfrage', record: r }); }}
          stammdatenList={(data as any).stammdaten ?? []}
        />
      )}
